...
def main() -> None:
    ...
    # print("\n0. ИНФОРМАЦИЯ О ДАННЫХ")
    # print("-" * 40)
    # print(f"Всего транзакций: {len(df)}")
    # print(f"Колонки: {list(df.columns)}")
    # print(f"Уникальные категории: {df['Категория'].unique()[:10] if 'Категория' in df.columns else 'Нет категорий'}")
    # print(f"Номера карт: {df['Номер карты'].unique()[:5] if 'Номер карты' in df.columns else 'Нет карт'}")
